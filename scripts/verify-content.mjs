...
const requiredPageGroups = [
  ['home page', ['acasa', 'home']],
  ['recipes page', ['retete', 'recipes']],
  ['guides page', ['articole', 'guides', 'articles']],
  ['author page', ['despre-autor', 'about-author']],
  ['contact page', ['contact']],
  ['privacy policy page', ['politica-de-confidentialitate', 'privacy-policy']],
  ['cookie policy page', ['politica-de-cookies', 'cookie-policy']],
  ['advertising/consent page', ['publicitate-si-consimtamant', 'advertising-and-consent']],
  ['editorial policy page', ['politica-editoriala', 'editorial-policy']],
  ['terms page', ['termeni-si-conditii', 'terms-and-conditions']],
  ['culinary disclaimer page', ['disclaimer-culinar', 'culinary-disclaimer']],
];

for (const [label, candidates] of requiredPageGroups) {
  if (!hasAnyPage(...candidates)) failures.push(`Missing required ${label}: expected one of ${candidates.join(', ')}`);
}

const aboutPage = pages.find((page) => {
  const slug = String(page.slug || '');
  return (/^despre-/.test(slug) || /^about-/.test(slug)) && !['despre-autor', 'about-author'].includes(slug);
});
...
if (countPostsByCategory('articole', 'guides', 'articles') !== articleCount) {
