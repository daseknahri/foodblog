...
function isAboutSiteSlug(slug) {
  return (/^(about|despre)-/.test(slug) && slug !== 'about-author' && slug !== 'despre-autor');
}
...
function checkPages() {
  ...
  const requiredGroups = [
    ['home page', ['home', 'acasa']],
    ['recipes page', ['recipes', 'retete']],
    ['guides page', ['guides', 'articole']],
    ['contact page', ['contact']],
    ['privacy policy page', ['privacy-policy', 'politica-de-confidentialitate']],
    ['cookie policy page', ['cookie-policy', 'politica-de-cookies']],
    ['advertising/consent page', ['advertising-and-consent', 'publicitate-si-consimtamant']],
    ['editorial policy page', ['editorial-policy', 'politica-editoriala']],
    ['terms page', ['terms-and-conditions', 'termeni-si-conditii']],
    ['culinary disclaimer page', ['culinary-disclaimer', 'disclaimer-culinar']],
  ];

  for (const [label, candidates] of requiredGroups) {
    if (!candidates.some((slug) => slugs.has(slug))) {
...
function isEditorialCategorySlug(slug) {
  return slug === 'articole' || slug === 'guides' || slug.includes('guide') || slug.includes('article');
}
