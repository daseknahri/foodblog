(function () {
  ...
  function scorePost(post, sourceWords, preferredCategories = []) {
    ...
    normalizedPreferredCategories.forEach((category) => {
      if (normalizedPostCategories.includes(category)) {
        score += 12;
      } else if (category !== 'guides' && normalizedPostCategories.length) {
        score -= 2;
      }
    });
  ...
  function isCleanupBoundaryHeading(line) {
    const heading = canonicalRecipeHeading(line);
    return heading === 'ingredients' || heading === 'steps' || heading === 'stop';
  }
  ...
  function cleanupRecipeDetailBlockInEditor() {
    ...
    if (!contentHasMarkup(html)) {
      ...
      String(html).split(/\r\n|\r|\n/).forEach((line) => {
        ...
        if (isCleanupBoundaryHeading(plain)) {
    ...
    Array.from(container.childNodes).forEach((node) => {
      ...
      const headingLike = /^h[1-6]$/.test(tag) || (tag === 'p' && isCleanupBoundaryHeading(plain));
  ...
  function structurePreservingSplitBlocks(blocks, parts) {
    ...
    const targetWords = Math.max(80, Math.ceil(totalWords / Math.max(1, parts * 2)));
    const output = [];
  ...
  function wordChunkBlocks(plain, targetChunks) {
    ...
    const chunkSize = Math.max(80, Math.ceil(words.length / Math.max(1, targetChunks)));
    const chunks = [];
