...
- Test `DISPLAY_AD_CARD_GRID_BASE64` only after post-body ads are stable. It appears after six real cards on recipe/guide/archive grids.
...
7. More display: add `reading_option` first, then `card_grid`, then below-content 320x50 or native only if readability remains good.
